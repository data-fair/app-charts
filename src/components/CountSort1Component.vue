...
export default {
  setup () {
    ...
    function cleanSearchParams () {
      ...
      if (!config.value.dataType.dynamicSort2 && urlSearchParams.count_sort_by_2) {
    ...
    onMounted(async () => {
      ...
      cleanSearchParams()
      const urlSortKey = urlSearchParams.count_sort_by_1
      const defaultSort = configSchema.definitions.sortCount.oneOf.find((option) => option.const === config.value.dataType.sort)?.title || configSchema.definitions.sortCount.default
