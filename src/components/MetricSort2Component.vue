...
export default {
  setup() {
    ...
    onMounted(async () => {
      ...
      cleanSearchParams()
      selectedSort.value = sortOptions.value.find((option) => option.key === urlSearchParams.metric_sort_by_2) || configSchema.definitions.sortMetric.oneOf.find((option) => option.const === configSchema.definitions.sortMetric.default).title
