  ...
  setup() {
    ...
    const applySort = (sortValue) => {
      ...
      if (selectedOption) {
        ...
        urlSearchParams.metric_sort_by_2 = selectedOption.key
      }
      appInfo.fetchData()
    }
    ...
    function cleanSearchParams() {
      ...
      if (!config.value.dataType.dynamicSort1 && urlSearchParams.metric_sort_by_1) {
    ...
    onMounted(async () => {
      ...
      cleanSearchParams()
      const urlSortKey = urlSearchParams.metric_sort_by_2
      const defaultSortOption = configSchema.definitions.sortMetric.oneOf.find(option => option.const === config.value.dataType.secondSort) || configSchema.definitions.sortMetric.oneOf[0]
